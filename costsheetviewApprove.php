        ...
        <div class="navview-content h-100">
            ...
            <div class="content-inner h-100" style="overflow-y: auto">
                <div class="row border-bottom bd-lightGray pl-1 mr-1 ribbed-lightGray" style="margin-left: 0px;">
                    <div class="cell-md-4">
                        <h4 class="dashboard-section-title text-center text-left-md w-100 content-title-big" data-group="work-order" data-page="<?=$pageOpt->currentPageClass()?>" style="font-size: 1.2rem; line-height: 1; font-weight: bold; text-shadow: 1px 1px 2px #fff;"><span class="icon"><span class="mif-assignment"></span></span> Cost Sheet Approve</h4>
                    </div>
                    <div class="cell-md-8">
...
as offerprice,published_status,approval_status,MPM_OFFER_PRICE,approve_manager_status,approve_manager_by,
                                ...
                                FROM inv.
                                    MRD_PRECOSTING_MASTER  MPM WHERE PUBLISHED_status=1 and DELETE_STATUS=0 AND APPROVE_MANAGER_STATUS=1 AND (MPM_EUSER = '$userId'
...
(SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'super admin')
                            ...
                            ?>
                                <tr class="text-center">
                                    ...
                                    <?php if ($row['APPROVAL_STATUS'] == 0 or $row['APPROVAL_STATUS']== null ){ ?>
                                        <a onclick="approvalData(this.id)" class="button small warning fg-white rounded mif-done" title="Click to Approve" id="<?php echo $row['MPM_NO'] ?>"></a> 
                                        <?php }else{ ?>
                                        <a class="button small success fg-white rounded mif-done_all" title="Already Approved" id="<?php echo $row['MPM_NO'] ?>"></a> 
                                        <?php } ?>
                                    </td>
