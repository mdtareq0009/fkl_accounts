...
if(!$auth->authUser()):
	...
	if(isset($_POST['formName'])):
		...
		elseif($_POST['formName'] == 'userfinder'):
			$fklid = $_POST['fklid'];
			if($accessoriesModel->checkDataExistence("SELECT vuserid FROM ADN_USERINFORMATION WHERE vempid = '$fklid'") == 'exist'):
		...
		elseif($_POST['formName'] == 'managerfinder'):
			$fklid = $_POST['fklid'];
			if($accessoriesModel->checkDataExistence("SELECT vemployeeid FROM hrm_employee WHERE vemployeeid = '$fklid'") == 'exist'):
				$userInfo = $accessoriesModel->getData("SELECT vempname FROM hrm_employee WHERE vemployeeid = '$fklid'");
				$response = array(
					'status' => 'success',
					'name' => $userInfo[0]['VEMPNAME']
