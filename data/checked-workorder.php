...
if ($auth->authUser()):
	if ($auth->verifyUserPermission('checked', 2)):
		...
		WITHIN GROUP (ORDER BY  ksorderinfo.vpart ASC), '([^,]+)(, \\1)+', '\\1') AS ksstylename, REGEXP_REPLACE(LISTAGG(ksorderinfo.vordernumber, ', ')
		WITHIN GROUP (ORDER BY ksorderinfo.norderid ASC), '([^,]+)(, \\1)+', '\\1') AS ksordernumber, ksorderinfo.vname AS ksbuyername 
		FROM accessories_workordermaster master LEFT JOIN accessories_workorderitems items ON items.nworkordermasterid = master.nid 
		...
		LEFT JOIN hr_employeeinfo@crypton checkedby ON checkedby.empid = master.vcheckeduser LEFT JOIN accessories_goods goods ON goods.nid = items.ngoodsid
		WHERE master.vstatus = 'publish' AND master.ncheckedstatus = 1 AND master.napprovedstatus = 0 AND master.nacceptencestatus = 0 
		AND master.nmerchandiserstatus=0
		AND master.nmerchandisermanagerstatus=0
		AND master.nmerchandisergmstatus=0
		AND master.npurchasegmstatus=0
		AND master.ndeletedstatus = 0 AND (master.vcreateduser = '$userid' OR master.vcreateduser IN ($managerFeature) OR master.vcreateduser IN ($subordinateFeature) OR master.vcreateduser IN ($subordinateFeature_2)
		OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userid') = 'admin' 
		OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userid') = 'super admin') GROUP BY master.nid, master.vponumber, 
		master.vblockorderinfo, master.vissue, master.vordernumberorfklnumber, master.vtype, master.vpublisheduser, employee.empname, 
		master.vpublishedat, master.ncheckedstatus, master.vcheckeduser, checkedby.empname, master.vapproveduser, approvedemployee.empname, 
		orderinfo.vname, ksorderinfo.vname ORDER BY master.nid DESC) where rownum <= 10000");
		$data = '{
		"header": [
		{
		"name": "purchasenumber",
		...
		if ($auth->verifyUserPermission('checked', 2)):
			$data .= '{
		"name": "merchandiser",
		"title": "<input type=\'checkbox\' data-role=\'checkbox\' data-style=\'2\' onchange=\'allCheck(\"checked-workorder-table\", $(this))\' data-cls-check=\'bd-white\' data-cls-checkbox=\'all-checked\' data-caption=\'\'>",
		"size": 18,
		"clsColumn": "text-center",
		"cls": "text-center"
		},';
		...
		if (is_array($masterData)):
			foreach ($masterData as $key => $datarow):
				...
				$data .= '"<a href=\'users.php?page=usersdetails&fklid=' . $datarow['VCHECKEDUSER'] . '\'>' . $datarow['CHECKEDUSER'] . '</a>",';
				if ($auth->verifyUserPermission('checked', 2)):
					$data .= '"<input type=\'checkbox\' data-role=\'checkbox\' value=\'' . $datarow['NID'] . '\' onchange=\'singleCheck(\"checked-workorder-table\", $(this), event)\' data-style=\'2\' data-cls-check=\'bd-dark\' data-cls-checkbox=\'single-check\' data-caption=\'\'>",';
				endif;
				$data .= '"' . ($auth->verifyUserPermission('checked', 2) == true ? '<a class=\'tool-button outline dark\' data-role=\'hint\' data-hint-position=\'left\' data-hint-text=\'Click to view details of work order <br>(<strong>' . $datarow['VPONUMBER'] . '</strong>)\' style=\'width: 26px;height: 26px;line-height: 26px;\' href=\'workorder.php?page=details&id=' . $datarow['NID'] . '\'><span class=\'mif-eye\' style=\'height: 22px; line-height: 22px; font-size: .9rem;\'></span></a> ' : '') . '' . ($auth->verifyUserPermission('checked', 2) == true ? '<a class=\'tool-button info\' data-role=\'hint\' data-hint-position=\'left\' data-hint-text=\'Print Work Order <br>(<strong>' . $datarow['VPONUMBER'] . '</strong>)\' style=\'width: 26px;height: 26px;line-height: 26px;\' href=\'reports/workorder/' . $datarow['NID'] . '\' target=\'_blank\'><span class=\'mif-printer\' style=\'height: 22px; line-height: 22px; font-size: .9rem;\'></span></a> ' : '') . ($auth->verifyUserPermission('publish workorder', 10) == true ? '<button type=\'button\' data-role=\'hint\' data-hint-position=\'left\' data-hint-text=\'Move to drafts work order page\' ' . ($datarow['VISSUE'] > 1 ? 'disabled' : '') . ' onclick=\'rowRollBack(' . $datarow['NID'] . ', \"published\", \"workorder\", $(this))\' class=\'tool-button yellow\' style=\'width: 26px;height: 26px;line-height: 26px;\'><span class=\'mif-refresh\' style=\'height: 22px; line-height: 22px;\'></span></button> ' : '') . '"';
