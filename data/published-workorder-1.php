...
if ($auth->authUser()):
	if ($auth->verifyUserPermission('publish workorder', 2)):
		...
		REGEXP_REPLACE(LISTAGG(UPPER(orderinfo.vstylename), ', ') WITHIN GROUP 
		(ORDER BY  orderinfo.vpart ASC), '([^,]+)(, \\1)+', '\\1') AS orderinfostylename, 
		...
		GROUP BY master.nid, master.vponumber, master.vblockorderinfo, master.vissue, master.vordernumberorfklnumber, master.vtype, master.vpublisheduser, employee.vempname, master.vpublishedat, master.ncheckedstatus, master.vcheckeduser, checkedby.vempname, orderinfo.vname, ksorderinfo.vname ORDER BY master.nid DESC) 
		where rownum <= 10000");
