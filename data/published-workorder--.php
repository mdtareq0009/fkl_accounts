...
if($auth->authUser()):
	 if($auth->verifyUserPermission('publish workorder', 2)):
		...
		REGEXP_REPLACE(LISTAGG(UPPER(orderinfo.vstylename), ', ') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(, \\1)+', '\\1') AS orderinfostylename,
		 ...
		 LEFT JOIN hrm_employee employee ON employee.vemployeeid = master.vpublisheduser 
		...
		master.vtype, 
		master.vpublisheduser, 
		employee.vempname, master.vpublishedat, master.ncheckedstatus, master.vcheckeduser, checkedby.vempname, orderinfo.vname, ksorderinfo.vname ORDER BY master.nid DESC) where rownum <= 6000");
		...
		if(is_array($masterData)):
			$superadmin = $accessoriesModel->getData("select vrole from ACCESSORIES_USERSPERMISSION where vfklid='$userid'");
			$superAdmin = $superadmin[0]['VROLE'];
			if($superAdmin != 'super admin'){
				// echo "Non super block ".$superAdmin;
				// test($superAdmin);
				$exist = $accessoriesModel->checkDataExistence("select vmanagerid from ACCESSORIES_USERSPERMISSION where vmanagerid='$userid'");
				if($exist == 'exist'){
					$buyerWiseManager = [];
					$buyer = $accessoriesModel->getData("select vmanagerid,buyer_nmae from ACCESSORIES_USERSPERMISSION where vmanagerid='$userid'");
					$buyerArray = explode(",",$buyer[0]['BUYER_NMAE']);
					$buyersName = $accessoriesModel->getData("select nbuyerid,vname from erp.mer_buyername");
					is_array($buyersName);
					foreach($buyersName as $buyerId){
						if(in_array($buyerId['NBUYERID'],$buyerArray))
						$buyerWiseManager[] = $buyerId['VNAME'];
					}
					//=========================================Buyer wise data=======================================================
			...
			// ===========================================================
			foreach ($masterData as $key => $datarow):
			$buyerNameForManager = isset($datarow['ORDERINFOBUYERNAME']) ? $datarow['ORDERINFOBUYERNAME'] : $datarow['KSBUYERNAME'];
			 if(in_array($buyerNameForManager,$buyerWiseManager))
			$test = $buyerNameForManager;
			if(isset($test))
			if(($test == $buyerNameForManager)){
			// ===========================================================
			if($first):
				$first = false;
			else:
			...
			if($datarow['VTYPE'] == 'Order number'):
				if(strtolower($datarow['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($datarow['VORDERNUMBERORFKLNUMBER']) == 'store'):
					...
					$buyerName = $datarow['ORDERINFOBUYERNAME']; //Original
