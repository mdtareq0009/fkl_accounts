...
MPT_TOTAL_TRIM_PRICE,MPO_TOTAL_PRICE,MPC_TOTAL_PROFIT,MPC_TOTAL_EXCES,MPC_TOTAL_CM,MPM_TOTAL_FABRIC_PRICE,MPM_TOTAL_TRIM_PRICE
,MPM_TOTAL_OTHER_PRICE,REVISE_STATUS,
MPM_TOTAL_MATERIAL_PRICE,MPM_TOTAL_CM_PRICE,MPM_TOTAL_CB_PRICE,MPM_PROFIT_PRICE,MPM_FOB_PRICE,MPM_UNIT_PRICE,MPM_TOTAL_PRICE
,MPM_REMARKS,MPM_PRICE_DEFF,MPM_OFFER_PRICE,
(SELECT vempname FROM erp.hrm_vw_employeeinfo U WHERE U.vemployeeid = MPM.MPM_EUSER) AS USER_NAME
,(SELECT bempsign FROM erppic.hrm_employeepicture U WHERE U.vemployeeid = MPM.MPM_EUSER) AS USER_SIGN
,(SELECT bempsign FROM erppic.hrm_employeepicture U WHERE U.vemployeeid = MPM.approval_by) AS APPROVAL_SIGN
,(SELECT bempsign FROM erppic.hrm_employeepicture U WHERE U.vemployeeid = MPM.approve_manager_by) AS ACCEPTED_SIGN
,(SELECT VNAME FROM ERP.MER_BUYERNAME MB WHERE MB.NBUYERID = MPM.MPM_BUYER_ID ) AS BUYER_NAME,
MRD_BUYER_STYLE_ID AS BUYER_STYLE,(select listagg(offer.offer_price,',') within group (order by offer.mpop_sl asc)
from inv.mrd_precosting_offer_price offer where offer.mpm_no = mpm.mpm_no)as offerprice,
...
(select listagg(distinct mPf.mrd_fab_ID ,',') within group(order by mpf.mpf_sl asc) AS fab_name from inv.mrd_precosting_fabric mpf where mpf.mpm_no='$masterId') as fab_name,(select round(sum(mpf_cadcon),5) as cadcon FROM inv.mrd_precosting_fabric cad where cad.mpm_no='$masterId') AS cadcon FROM inv.MRD_PRECOSTING_MASTER  MPM where mpm.mpm_no='$masterId'");

// $queryResult = $accessoriesModel->getData("SELECT 
// MPM_SL,MPM_EUSER,MPM_NO,
// TO_CHAR(MPM_EDATE,'dd-MM-yyyy') AS MPM_EDATE,MPM_ETIME,MPM_NO,TO_DATE(MPM_CDATE,'dd-MM-yyyy') AS MPM_CDATE,
// MPM_BUYER_ID,MRD_BUYER_STYLE_ID,MRD_BUYER_SEASON_ID,MRD_BUYER_DEPT_ID,MPM_PACK_TYPE,MPM_PACK_NUMBER,MPM_ORDER_QTY,MPF_TOTAL_FAB_PRICE,
// MPT_TOTAL_TRIM_PRICE,MPO_TOTAL_PRICE,MPC_TOTAL_PROFIT,MPC_TOTAL_EXCES,MPC_TOTAL_CM,MPM_TOTAL_FABRIC_PRICE,MPM_TOTAL_TRIM_PRICE,MPM_TOTAL_OTHER_PRICE,REVISE_STATUS,
...
// (SELECT vempname FROM erp.hrm_vw_employeeinfo U WHERE U.vemployeeid = MPM.MPM_EUSER) AS USER_NAME,(SELECT VNAME FROM ERP.MER_BUYERNAME MB WHERE MB.NBUYERID = MPM.MPM_BUYER_ID ) AS BUYER_NAME,(SELECT bempsign FROM erppic.hrm_employeepicture U WHERE U.vemployeeid = MPM.MPM_EUSER) AS USER_SIGN,
// MRD_BUYER_STYLE_ID AS BUYER_STYLE,(select listagg(offer.offer_price,',') within group (order by offer.mpop_sl asc) from inv.mrd_precosting_offer_price offer where offer.mpm_no = mpm.mpm_no)as offerprice,
// (SELECT VSEASSONNAME FROM ERP.MER_SEASSONNAME MS WHERE MS.NSEASSONCODE = MPM.MRD_BUYER_SEASON_ID) AS BUYER_SEASON,
// (SELECT MD.VDEPTNAME FROM ERP.MER_DEPARTMENT MD WHERE MD.NDEPTID = MPM.MRD_BUYER_DEPT_ID AND MPM.MPM_BUYER_ID=MD.NBUYERID) AS BUYER_DEPT,
// (SELECT mip.mrd_item_pack_name FROM inv.mrd_item_pack mip WHERE mip.mrd_item_pack_id = MPM.MPM_PACK_TYPE) AS pack_type,
// (select listagg(mPf.mrd_fab_ID ,',') 
// within group(order by mpf.mpf_sl asc) AS fab_name from inv.mrd_precosting_fabric mpf where mpf.mpm_no='$masterId') as fab_name,(select round(sum(mpf_cadcon),5) as cadcon FROM inv.mrd_precosting_fabric cad where cad.mpm_no='$masterId') AS cadcon FROM inv.
...
<body>
		...
		<div class="float-left m-0">
			...
			<table class="table row-border cell-border table-border subcompact">
				...
				<tr>
					...
					<!-- <td>:</td> -->
					<td><?php echo $queryResult[0]['MRD_BUYER_STYLE_ID']; ?></td>
				</tr>
	...
	<div class="float-right">
		...
		<table class="table table-border row-border cell-border subcompact">
			<tr>
				...
				<!-- <td>:</td> -->
				<?php if($queryResult[0]['REVISE_STATUS'] != null or $queryResult[0]['REVISE_STATUS'] !=0){ ?>
				<td><strong><?php echo $queryResult[0]['MPM_NO']?> <span class="fg-red"><?php echo '(REV-'.$queryResult[0]['REVISE_STATUS'].')'; ?></span></strong></td>
				<?php } else{ ?>
				<td><strong><?php echo $queryResult[0]['MPM_NO']?></strong></td>
				<?php } ?>
			</tr>
			<tr>
	...
	<!-- <div class="container1"> -->
		<!-- <div class="relative-div"> -->
			...
			<table class="table row-border cell-border table-border subcompact">
				<caption align="center"><b>MARKETING CONSUMPTION</b></caption>
				<thead>
				...
				<tbody>
					<?php 
						$fabTable = $accessoriesModel->getData("SELECT MPF_SL,MPF.MPM_NO, MRD_ITEM_ID,(SELECT MRD_ITEM_NAME FROM INV.MRD_ITEM MI
						WHERE MI.MRD_ITEM_ID = MPF.MRD_ITEM_ID) AS MRD_ITEM_NAME,MRD_FAB_ID,(SELECT MRD_FAB_NAME FROM INV.MRD_FAB MF 
						WHERE MF.MRD_FAB_ID= MPF.MRD_FAB_ID) AS MRD_FAB_NAME,MRD_COLOR_ID,(SELECT MRD_COLOR_NAME FROM INV.MRD_COLOR MC
						WHERE MC.MRD_COLOR_ID= MPF.MRD_COLOR_ID) AS MRD_COLOR_NAME,MRD_YARN_COUNT_ID,(SELECT MRD_YARN_COUNT_NAME 
						FROM INV.MRD_YARN_COUNT MYC WHERE MYC.MRD_YARN_COUNT_ID= MPF.MRD_YARN_COUNT_ID) AS MRD_YARN_COUNT_NAME
						,(SELECT mrd_fab_gsm FROM INV.MRD_FAB_gsm MF 
						WHERE MF.MRD_FAB_gsm_sl= MPF.mpf_gsm) AS MPF_GSM,MPF_CADCON,MPF_RATIO,MPF_GREIGE_FABRIC,MPF_YARN_PRICE,MPF_KNIT_PRICE,MPF_DYEING_PRICE
						,MPF_FAB_COST,MPF_AOP_YD_PRICE,MPF_AOP_COST,MPF_FAB_PRICE
						FROM INV.MRD_PRECOSTING_FABRIC MPF WHERE MPF.MPM_NO='$masterId'");
						// $fabTable = $accessoriesModel->getData("SELECT MPF_SL,MPF.MPM_NO, MRD_ITEM_ID,(SELECT MRD_ITEM_NAME FROM INV.MRD_ITEM MI WHERE MI.MRD_ITEM_ID = MPF.MRD_ITEM_ID) AS MRD_ITEM_NAME,MRD_FAB_ID,(SELECT MRD_FAB_NAME FROM INV.MRD_FAB MF WHERE MF.MRD_FAB_ID= MPF.MRD_FAB_ID) AS MRD_FAB_NAME,MRD_COLOR_ID,(SELECT MRD_COLOR_NAME FROM INV.MRD_COLOR MC WHERE MC.MRD_COLOR_ID= MPF.MRD_COLOR_ID) AS MRD_COLOR_NAME,MRD_YARN_COUNT_ID,(SELECT MRD_YARN_COUNT_NAME FROM INV.MRD_YARN_COUNT MYC WHERE MYC.MRD_YARN_COUNT_ID= MPF.MRD_YARN_COUNT_ID) AS MRD_YARN_COUNT_NAME,MPF_GSM,MPF_CADCON,MPF_RATIO,MPF_GREIGE_FABRIC,MPF_YARN_PRICE,MPF_KNIT_PRICE,MPF_DYEING_PRICE,MPF_FAB_COST,MPF_AOP_YD_PRICE,MPF_AOP_COST,MPF_FAB_PRICE FROM INV.MRD_PRECOSTING_FABRIC MPF WHERE MPF.MPM_NO='$masterId'");
					// echo '<pre>';
					// print_r($fabTable);
					// exit;
					$f = 0;
					...
					<tr>
						...
						<td><?php echo $fab['MRD_FAB_ID'] ?></td>
						<td><?php echo $fab['MRD_COLOR_ID'] ?></td>
	...
	<!-- <div class="cell-md-12 cell-sm-12"> -->
		<div class="row">
			...
			<div class="float-right mt-5" style="width: 260px;margin-left:20px;">
				...
				?>
					<img src="./reports/no_image.png" alt="No Image" class="mt-5" width="260px" height="220px" style="line-height: 0px;">
				<?php } ?>
				</table>
			</div>
		</div>
	...
	<footer >	
		<div id="fotter">
			...
			<table>
				<tr align="center">
					<!-- <td width="1%">	<u> <?php 
					// if( $queryResult[0]['USER_SIGN'] != null){
					// 	echo "<img style='display:block; width:150px;height:70px;' id='base64image' src='data:image/png;base64, ".base64_encode($queryResult[0]['USER_SIGN']->load())."' />";
					// }else{
					...
					<td width="1%"><b>
					<?php 
					if( $queryResult[0]['ACCEPTED_SIGN'] != null){
						echo "<img style='display:block; width:150px;height:50px;' id='base64image' src='data:image/png;base64, ".base64_encode($queryResult[0]['ACCEPTED_SIGN']->load())."' />";
					}else{
						echo "____________________";
					}
					?></b></td>
					...
					<td width="1%"><b>
					<?php 
					if( $queryResult[0]['APPROVAL_SIGN'] != null){
						echo "<img style='display:block; width:150px;height:70px;' id='base64image' src='data:image/png;base64, ".base64_encode($queryResult[0]['APPROVAL_SIGN']->load())."' />";
					}else{
