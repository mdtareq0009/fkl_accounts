								...
								if($accessoriesModel->insertData('ACCESSORIES_WORKORDERITEMDATA', $item) == 'inserted'):
									...
									if(is_array($sizeData)):
								    	...
								    	foreach ($sizeData[$itemDatakey] as $sizeName => $sizeQty):
								    		...
								    		if($accessoriesModel->insertDataManualCommit('ACCESSORIES_WORKORDERSIZEQTY', $size) == 'not inserted'):
								    			array_push($checkError, 'errors');
											endif;
								        endforeach;
							    	endif;
		...
		elseif($_POST['formName'] == 'workorderupdate'):
			if($db->csrfVerify($_POST['csrf']) == 'success'):
				...
				//if($accessoriesModel->deleteSingleRow("ACCESSORIES_WORKORDERMASTER", "nid = $masterId")):
				    if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_WORKORDERMASTER", "nid = $masterId") == 'failed'):
				    	array_push($checkError, 'errors');
				    endif;
			 ...
			 //        die();
				...
				if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_WORKORDERITEMS", "NWORKORDERMASTERID = $masterId") == 'failed'):
					array_push($checkError, 'errors');
				endif;
				if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_WORKORDERITEMDATA", "NWORKORDERITEMSID IN ($exItemId)") == 'failed'):
					array_push($checkError, 'errors');
				endif;
				if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_CUSTOMCOLUMNVALUE", "NGRIDITEMDATAID IN ($exGridId)") == 'failed'):
					array_push($checkError, 'errors');
				endif;
				if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_WORKORDERSIZEQTY", "NWORKORDERITEMSDATAID IN ($exGridId)") == 'failed'):
					array_push($checkError, 'errors');
				endif;
				if($accessoriesModel->deleteSingleRowManualCommit("ACCESSORIES_WORKORDERFORMSETTING", "NWORKORDERITEMSID IN ($exItemId)") == 'failed'):
					array_push($checkError, 'errors');
				endif;
