      		...
      		if($accessoriesModel->checkDataExistence("SELECT nid FROM accessories_workordermaster WHERE nid = $id AND ndeletedstatus = 0 AND vstatus = 'publish' AND nissuestatus = 1") == 'exist'):
        		...
        		if(strtolower($masterData[0]['VTYPE']) == 'order number'):
          			...
          			if(strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'store'):
          				...
          				$orderInfo = $accessoriesModel->getData("SELECT REGEXP_REPLACE(LISTAGG(UPPER(vstylename), ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS stylename, REGEXP_REPLACE(LISTAGG(orderinfo.kimballno, ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS kimballno, LISTAGG(nks_id, ', ') WITHIN GROUP (ORDER BY orderinfo.norderid ASC) AS ksid, orderinfo.vname AS buyername, orderinfo.vseassonname, orderinfo.vdeptname FROM ERP.mer_vw_orderinfo orderinfo LEFT JOIN erp.mer_ks_master kmaster ON kmaster.nordercode = orderinfo.norderid WHERE orderinfo.vordernumber = '$orderNumber' AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') <= 180 AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') >= 0 GROUP BY orderinfo.vname, orderinfo.vseassonname, orderinfo.vdeptname ORDER BY orderinfo.vseassonname DESC");
        		...
        		class PDF extends FPDF{
        			...
        			public function Footer() {
          				...
          				global $fklNumber;
          				global $AOP;
          				$purchase = $AOP[0]['ITEMNAME'] == 'AOP' ? 'A.G.M. (Knitting)' : 'G.M. (Purchase)'; 
          				if(!isset($this->footerset[$this->page])) {
            					...
            					if(strtolower($fklNumber) == 'store' || strtolower($orderNumber) == 'store'):
	            					...
	            					$tableFooter = array(
	              						...
	              						'columnWidth'         => array($cellWidth, $cellWidth, $cellWidth, $cellWidth, $cellWidth),
	              						'columnFontSize'      => 8,
	              						'columnFontWeight'    => '',
	            					...
	            					$tableFooter = array(
	              						'columnName'          => array($createdUserName, 'Checked By', 'Manager (Store)', 'A.G.M. (Inventory)', 'Approved By'),
	              						'columnWidth'         => array($cellWidth, $cellWidth, $cellWidth, $cellWidth, $cellWidth),
	              						'columnFontSize'      => 8,
	              						'columnFontWeight'    => '',
	              						'columnHeight'        => 3, //set column padding
	            					...
	            					$tableFooter = array(
	              						'columnName'          => array('Prepared By', '---------------', '-----------------------------', '-------------------', '-------------------------------------------', '-------------------', '----------------'),
	              						'columnWidth'         => array($cellWidth-8, $cellWidth-8, $cellWidth+6, $cellWidth-1, $cellWidth+19, $cellWidth, $cellWidth-5),
	              						'columnFontSize'      => 8,
	              						'columnFontWeight'    => '',
	              						'columnHeight'        => 3, //set column padding
	              						'columnAlign'         => 'C', //options center, left, right
	            					...
	            					$tableFooter = array(
	              						'columnName'          => array($createdUserName, 'Checked By', 'Asst. Manager / Manager (Merchandising)', 'Senior Manager (Merchandising)', 'G.M. (Merchandising & Marketing)', $purchase, 'Approved By'),
	              						'columnWidth'         => array($cellWidth-8, $cellWidth-8, $cellWidth+6, $cellWidth-1, $cellWidth+19, $cellWidth, $cellWidth-5),
	              						'columnFontSize'      => 8,
	              						'columnFontWeight'    => '',
	              						'columnHeight'        => 3, //set column padding
