      		...
      		if($accessoriesModel->checkDataExistence("SELECT nid FROM accessories_workordermaster WHERE nid = $id AND ndeletedstatus = 0 AND vstatus = 'draft'") == 'exist'):
        		...
        		if(strtolower($masterData[0]['VTYPE']) == 'order number'):
          			...
          			if(strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'store'):
          				...
          				$orderInfo = $accessoriesModel->getData("SELECT REGEXP_REPLACE(LISTAGG(UPPER(vstylename), ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS stylename, REGEXP_REPLACE(LISTAGG(orderinfo.kimballno, ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS kimballno, LISTAGG(nks_id, ', ') WITHIN GROUP (ORDER BY orderinfo.norderid ASC) AS ksid, orderinfo.vname AS buyername, orderinfo.vseassonname, orderinfo.vdeptname FROM ERP.mer_vw_orderinfo orderinfo LEFT JOIN erp.mer_ks_master kmaster ON kmaster.nordercode = orderinfo.norderid WHERE orderinfo.vordernumber = '$orderNumber' AND (SELECT max(TO_DATE(dshipdate, 'dd/mm/yyyy')) FROM erp.mer_vw_orderinfo WHERE vordernumber = '$orderNumber') >= TRUNC(SYSDATE) GROUP BY orderinfo.vname, orderinfo.vseassonname, orderinfo.vdeptname ORDER BY orderinfo.vseassonname DESC");
         				...
         				//$this->Cell(0, 20, $this->Image('report-watermark.jpg',  $this->GetPageWidth()/2-50, 85, 110, 110), 0, 0, 'L', false );
          				...
          				if($this->PageNo() == 1){
            					...
            					if(strtolower($fklNumber) == 'store' || strtolower($orderNumber) == 'store'):
	            					...
	            					// $this->Ln();
	            					$tableHeader = array(
	              						'columnName'          => array('To', strlen($masterData[0]['SUPPLIER']) > 30 ? substr(htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES),0,29)."..." : htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES), '', 'Season', $orderInfo[0]['VSEASSONNAME']),
	              						'columnWidth'         => array(15, 48, 1, 22, 52),
	              						'columnBackground'    => array(array('r'=> 222, 'g'=>222, 'b' => 222), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=> 222, 'g'=>222, 'b' => 222)),
	              						'columnBorder'        => array('r' => 255, 'g' => 255, 'b' => 255),
	             						'columnFontSize'      => 8.5,
