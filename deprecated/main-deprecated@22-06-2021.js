  ...
  $('.itemsevent select').on('change', function(evt){
    ...
    $('.itemsevent select :selected').each(function(index){
      ...
      if($('.gridappender').find('#'+trid).length == 0){
        ...
        dataTableCreate += "<table class='table subcompact cell-border table maingrid bg-white itemtable "+trid+" manual-data' data-uniqueid='"+id+"'>";
        dataTableCreate += "<tr>";
  ...
  $('.gridappender').on('click', '.parameters input', function(evt){
    ...
    if(ownObj.is(':checked')){
      ...
      if(data.toLowerCase() == 'converter'){
        ...
        $('#convertiontype1').text(getItemName+' ('+conUnit+') Per Garment');
        $('#convertiontype2').text('Garment(s) Per '+getItemName+' ('+conUnit+')');
        $('#convertiontype1').attr('value', getItemName+' ('+conUnit+') Per Garment');
        $('.extraCalAdded').empty();
        $('.extraCalAdded').append(extraFieldAdded);
        extraFieldAdded = '';
        $('#convertiontype2').attr('value', 'Garment(s) Per '+getItemName+' ('+conUnit+')');
     ...
     $('#'+parentDivId).find('.maingrid tr').each(function(index){
      ...
      if(ownObj.val().toLowerCase() == 'color wise qty' || ownObj.val().toLowerCase() == 'size wise qty' || ownObj.val().toLowerCase() == 'kimball & color wise qty' || ownObj.val().toLowerCase() == 'color & size wise qty' || ownObj.val().toLowerCase() == 'kimball/color/size wise qty'  || ownObj.val().toLowerCase() == 'garments qty'){
...
function checkvalidformat(ownClass){
  ...
  for (var i = 0;  i < ownClass[0].files.length; i++) {
    if($.inArray(ownClass[0].files[i].name.substr(-3), format) == -1 ){
