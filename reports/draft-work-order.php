      		...
      		if($accessoriesModel->checkDataExistence("SELECT nid FROM accessories_workordermaster WHERE nid = $id AND ndeletedstatus = 0 AND vstatus = 'draft'") == 'exist'):
        		...
        		if(strtolower($masterData[0]['VTYPE']) == 'order number'):
          			...
          			if(strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'store'):
          				...
          				$orderInfo = $accessoriesModel->getData("SELECT REGEXP_REPLACE(LISTAGG(distinct Initcap(vstylename), ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS stylename, REGEXP_REPLACE(LISTAGG(orderinfo.kimballno, ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS kimballno, LISTAGG(nks_id, ', ') WITHIN GROUP (ORDER BY orderinfo.norderid ASC) AS ksid, orderinfo.vname AS buyername, orderinfo.vseassonname, orderinfo.vdeptname FROM ERP.mer_vw_orderinfo orderinfo LEFT JOIN erp.mer_ks_master kmaster ON kmaster.nordercode = orderinfo.norderid WHERE orderinfo.vordernumber = '$orderNumber' AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') <= 180 AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') >= 0 GROUP BY orderinfo.vname, orderinfo.vseassonname, orderinfo.vdeptname ORDER BY orderinfo.vseassonname DESC");
        		...
        		class PDF extends FPDF{
          			public function Header(){
         				...
         				//$this->Cell(0, 20, $this->Image('report-watermark.jpg',  $this->GetPageWidth()/2-50, 85, 110, 110), 0, 0, 'L', false );
          				...
          				if($this->PageNo() == 1){
            					...
            					$this->SetX($this->GetPageWidth() - 59);
            					$tableHeader = array(
             						'columnName'          => array('P.O. Number', 'Draft'),
            					...
            					$this->SetX($this->GetPageWidth() -59);
            					$this->SetFont('Times','',8.5);
            					$this->Cell(49, 5, 'Draft', 1, 1, 'C', false );
        			...
        			public function Footer() {
          				...
          				if(!isset($this->footerset[$this->page])) {
            					...
            					//$this->Ln(2);
            					$tableFooter = array(
              						'columnName'          => array($createdUserName),
              						'columnWidth'         => array($cellWidth),
              						'columnFontSize'      => 8,
              						'columnFontWeight'    => '',
              						'columnHeight'        => 3, //set column padding
            					...
            					$this->TextWithDirection(6, $this->GetPageHeight() - 10, 'Draft Work Order', 'U');
            					// $this->SetTextColor(0, 0, 0);
      			...
      			$pdf = new PDF();
      			$pdf->SetTitle('(W.O. - '.$masterData[0]['NID'].') Draft Workorder');
      			$pdf->AliasNbPages();
      			$pdf->AddPage('P', 'A4');
      			$pdf->Ln(2);
                ...
                $itemNameMultiple = array();
      			...
      			$pdf->SetKeywords("Accessories Draft workorder");
      			$pdf->SetSubject("Draft Workorder / W.O-".$masterData[0]['NID']);
      			$pdf->output('', strtolower(str_replace(' ', '-', implode('-',$itemNameMultiple)).'-W-O-'.$masterData[0]['NID']).'-'.strtotime(date("Y-m-d H:i:s")).'.pdf');
    		else:
      			$auth->redirect404();
    		endif;
  	else:
