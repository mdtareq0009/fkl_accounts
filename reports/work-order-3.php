...
if($auth->authUser()):
	if($auth->verifyUserPermission('publish workorder', 6) || $auth->verifyUserPermission('approved workorder', 6) || $auth->verifyUserPermission('accepted workorder', 6) || $auth->verifyUserPermission('all workorder', 6)):
      		...
      		if($accessoriesModel->checkDataExistence("SELECT nid FROM accessories_workordermaster WHERE nid = $id AND ndeletedstatus = 0 AND vstatus = 'publish' AND nissuestatus = 1") == 'exist'):
        		...
        		$masterData = $accessoriesModel->getData("SELECT master.nid, master.vponumber, master.vblockorderinfo,item.ngoodsid,master.vordernumberorfklnumber, master.vtype, supplier.vname AS supplier, master.vtodate, master.vattn, master.vform, master.vgarmentsqty, master.vdeliverydate, master.vcreatedat, master.vcreateduser, master.vorderdetails, master.vextranotes, master.vissue FROM accessories_workordermaster master LEFT JOIN accessories_suppliers supplier ON supplier.nid = master.nsupllierid left join accessories_workorderitems item on item.nworkordermasterid=master.nid WHERE master.nid = $id AND master.vstatus = 'publish' AND master.ndeletedstatus = 0 AND master.nissuestatus = 1");
        		...
        		if(strtolower($masterData[0]['VTYPE']) == 'order number'):
          			...
          			if(strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'store'):
          				...
          				$orderInfo = $accessoriesModel->getData("SELECT REGEXP_REPLACE(LISTAGG(distinct initcap(vstylename), ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS stylename, REGEXP_REPLACE(LISTAGG(orderinfo.kimballno, ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS kimballno, LISTAGG(nks_id, ', ') WITHIN GROUP (ORDER BY orderinfo.norderid ASC) AS ksid, orderinfo.vname AS buyername, orderinfo.vseassonname, orderinfo.vdeptname FROM ERP.mer_vw_orderinfo orderinfo LEFT JOIN erp.mer_ks_master kmaster ON kmaster.nordercode = orderinfo.norderid WHERE orderinfo.vordernumber = '$orderNumber' AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') <= 365 AND to_date(orderinfo.dshipdate,'DD/MM/YYYY') - to_date('$createdat','DD/MM/YYYY') >= 0 GROUP BY orderinfo.vname, orderinfo.vseassonname, orderinfo.vdeptname ORDER BY orderinfo.vseassonname DESC");
        		...
        		elseif(strtolower($masterData[0]['VTYPE']) == 'fkl number'):
         			...
         			if(strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'block' || strtolower($masterData[0]['VORDERNUMBERORFKLNUMBER']) == 'store'):
	          			...
	          			$orderInfo = $accessoriesModel->getData("SELECT REGEXP_REPLACE(LISTAGG (distinct initcap(vstylename), ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS stylename, REGEXP_REPLACE(LISTAGG(orderinfo.kimballno, ',') WITHIN GROUP (ORDER BY  orderinfo.vpart ASC), '([^,]+)(,\\1)+', '\\1') AS kimballno, LISTAGG(orderinfo.vordernumber, ', ') WITHIN GROUP (ORDER BY orderinfo.norderid ASC) AS ordernumber,  orderinfo.vname AS buyername, orderinfo.vseassonname, orderinfo.vdeptname FROM ERP.mer_ks_master kmaster LEFT JOIN ERP.mer_vw_orderinfo orderinfo ON orderinfo.norderid = kmaster.nordercode WHERE kmaster.nks_id IN ($fklCon) GROUP BY orderinfo.vname, orderinfo.vseassonname, orderinfo.vdeptname ORDER BY orderinfo.vseassonname DESC");
        		...
        		class PDF extends FPDF{
         				...
         				//$this->Cell(0, 20, $this->Image('report-watermark.jpg',  $this->GetPageWidth()/2-50, 85, 110, 110), 0, 0, 'L', false );
          				...
          				if($this->PageNo() == 1){
            					...
            					if(strtolower($fklNumber) == 'store' || strtolower($orderNumber) == 'store'):
	            					...
	            					// $this->Ln();
	            					$tableHeader = array(
	              						'columnName'          => array('To', strlen($masterData[0]['SUPPLIER']) > 30 ? substr(htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES),0,29)."..." : htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES), '','Garments Qty', $masterData[0]['VGARMENTSQTY'].' Pcs'),
	            					...
	            					$tableHeader = array(
	              						'columnName'          => array('Date', $masterData[0]['VTODATE'], '', 'Style / Ref.', htmlspecialchars_decode($orderInfo[0]['STYLENAME'],ENT_QUOTES)),
	              						'columnWidth'         => array(15, 47, 2, 22, 58),
	              						'columnBackground'    => array(array('r'=> 222, 'g'=>222, 'b' => 222), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=> 222, 'g'=>222, 'b' => 222)),
	              						'columnBorder'        => array('r' => 255, 'g' => 255, 'b' => 255),
	              						'columnFontSize'      => 8.5,
	            					...
	            					$this->Ln();
	            					$tableHeader = array(
	              						'columnName'          => array('From', strlen($masterData[0]['VFORM']) > 30 ? substr($masterData[0]['VFORM'],0,29)."..." : $masterData[0]['VFORM'], '',$masterData[0]['NGOODSID']!=218?'Garments Qty':'Fabric Qty', $masterData[0]['NGOODSID']!=218? $masterData[0]['VGARMENTSQTY'].' Pcs':$masterData[0]['VGARMENTSQTY'].' Kg'),
	              						'columnWidth'         => array(15, 48, 1, 22, 52),
	              						'columnBackground'    => array(array('r'=> 222, 'g'=>222, 'b' => 222), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=> 222, 'g'=>222, 'b' => 222)),
	              						'columnBorder'        => array('r' => 255, 'g' => 255, 'b' => 255),
	              						'columnFontSize'      => 8.5,
        			...
        			public function Footer() {
          				...
          				$arrayMrdSignature1 = array('dashed' => array('Prepared By', '---------------', '-----------------------------', '-------------------', '-------------------------------------------', '-------------------', '----------------'), 'cellwidth' => array($cellWidth-8, $cellWidth-8, $cellWidth+6, $cellWidth-1, $cellWidth+19, $cellWidth, $cellWidth-5), 'signDesignation' => array($createdUserName, 'Checked By', 'Senior Merchandiser / Asst. Manager', ' Manager / Senior Manager (Merchandising)', 'G.M. (Merchandising & Marketing)', 'D.G.M. (Purchase)', 'Approved By'));
          				$arrayMrdSignature2 = array('dashed' => array('Prepared By', '---------------', '-----------------------------', '-------------------', '-------------------', '-------------------------------------------', '----------------'), 'cellwidth' => array($cellWidth-8, $cellWidth-8, $cellWidth+6, $cellWidth-1, $cellWidth, $cellWidth+19, $cellWidth-5), 'signDesignation' => array($createdUserName, 'Checked By', 'Senior Merchandiser / Asst. Manager', ' Manager / Senior Manager (Merchandising)', 'A.G.M. (Knitting)', 'G.M. (Merchandising & Marketing)', 'Approved By'));
          				...
          				if(!isset($this->footerset[$this->page])) {
            					...
            					if(strtolower($fklNumber) == 'store' || strtolower($orderNumber) == 'store'):
            						...
            						$arrayAccStoreSignature = array('dashed' => array('Prepared By', '---------------', '--------------------', '----------------------', '----------------'), 'cellwidth' => array($cellWidth, $cellWidth, $cellWidth, $cellWidth, $cellWidth), 'signDesignation' => array($createdUserName, 'Checked By', 'Manager (Store)', 'A.G.M. (Inventory)', 'Approved By'));
	            					$tableFooter = array(
        			...
        			foreach ($numberofTable as $key => $value):
        				array_push($itemNameMultiple, $value['ITEMNAME']);
          				...
          				if($workorderOpt->checkColumnExist($tableId, 'NROWGARMENTSQTY', 'ACCESSORIES_WORKORDERITEMDATA') == 1):
							if($AOP[0]['ITEMNAME']=='Foreign Fabric'){
