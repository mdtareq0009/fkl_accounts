...
if($auth->authUser()):
	if($auth->verifyUserPermission('publish workorder', 6) || $auth->verifyUserPermission('approved workorder', 6) || $auth->verifyUserPermission('accepted workorder', 6) || $auth->verifyUserPermission('all workorder', 6) || $auth->verifyUserPermission('checked', 6)):
      		...
      		if($accessoriesModel->checkDataExistence("SELECT nid FROM accessories_workordermaster WHERE nid = $id AND ndeletedstatus = 0 AND vstatus = 'publish' AND nissuestatus = 1") == 'exist'):
				...
				// $checkedUser = $masterData[0]['VCHECKEDUSER'];
         				...
         				//$this->Cell(0, 20, $this->Image('report-watermark.jpg',  $this->GetPageWidth()/2-50, 85, 110, 110), 0, 0, 'L', false );
          				...
          				if($this->PageNo() == 1){
            					...
            					if(strtolower($fklNumber) == 'store' || strtolower($orderNumber) == 'store'):
	            					...
	            					// $this->Ln();
	            					$tableHeader = array(
	              						'columnName'          => array('To', strlen($masterData[0]['SUPPLIER']) > 30 ? substr(htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES),0,29)."..." : htmlspecialchars_decode($masterData[0]['SUPPLIER'], ENT_QUOTES), '', $itemName[0]['ITEMNAME']=='Yarn'?'Yarn Qty':'Garments Qty.', $itemName[0]['ITEMNAME']=='Yarn'?$masterData[0]['VGARMENTSQTY'].' KG':$masterData[0]['VGARMENTSQTY'].' Pcs'),
	            					...
	            					$this->Ln();
	            					$tableHeader = array(
	              						'columnName'          => array('From', strlen($masterData[0]['VFORM']) > 30 ? substr($masterData[0]['VFORM'],0,29)."..." : $masterData[0]['VFORM'], '', $itemName[0]['ITEMNAME']=='Yarn'?'Yarn Qty':'Garments Qty.', $itemName[0]['ITEMNAME']=='Yarn'?$masterData[0]['VGARMENTSQTY'].' KG':$masterData[0]['VGARMENTSQTY'].' Pcs'),
	              						'columnWidth'         => array(15, 48, 1, 22, 52),
	              						'columnBackground'    => array(array('r'=> 222, 'g'=>222, 'b' => 222), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=>255, 'g'=>255, 'b'=>255), array('r'=> 222, 'g'=>222, 'b' => 222)),
	              						'columnBorder'        => array('r' => 255, 'g' => 255, 'b' => 255),
	              						'columnFontSize'      => 8.5,
