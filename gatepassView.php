        ...
        <div class="navview-content h-100">
            ...
            <div class="content-inner h-100" style="overflow-y: auto">
                <div class="row border-bottom bd-lightGray pl-1 mr-1 ribbed-lightGray" style="margin-left: 0px;">
                    <div class="cell-md-4">
                        <h4 class="dashboard-section-title text-center text-left-md w-100 content-title-big" data-group="work-order" data-page="<?=$pageOpt->currentPageClass()?>" style="font-size: 1.2rem; line-height: 1; font-weight: bold; text-shadow: 1px 1px 2px #fff;"><span class="icon"><span class="mif-assignment"></span></span> Gate Pass</h4>
                    </div>
                    <div class="cell-md-8">
                    ...
                    <div class="cell-md-12">
                        <div class="row">
                                <div class="cell-md-2 cell-sm-2 cell-sm-2 float-left">
                                    <input type="text" name="date1" id="date1" data-role="calendarpicker" data-cls-calendar="compact" data-format="%d-%m-%Y" data-clear-button="true" data-input-format="%d-%m-%y" value="<?=date('d-m-Y')?>" class="input-small">
                                </div>
                                <div class="cell-md-2 cell-sm-2 cell-lg-2">
                                    <input type="text" name="date2" id="date2" class="input-small" data-role="calendarpicker" data-cls-calendar="compact" data-format="%d-%m-%Y" data-clear-button="true" data-input-format="%d-%m-%y" value="<?=date('d-m-Y')?>">
                                </div>
                                <div class="cell-md-2 cell-sm-2 cell-lg-2">
                    ...
                    <!-- <div class="container" id="print-js"> -->
                        ...
                        <!-- <table class="table row-border cell-border border row-hover cell-hover subcompact" data-rownum="true" data-role="table"> -->
                            ...
                            <thead>
                                <!-- <th>Sl</th>						 -->
                                <?php
                                    if($auth->verifyUserPermission('gatepass', 4)){
                                ?>
                                <th class="text-center sortable-column">D.H. Receive</th>
                                <?php } ?>						
                            ...
                            <?php
                                 ...
                                 if(isset($_POST['submit'])){
                                    if(!empty($gp_no)){
                                    $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo,returnable,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY,m.return_date
                                    ,(SELECT empname FROM vw_all_employeeinformation@crypton U WHERE U.empid = m.userid) AS USER_NAME
                                    from gatepass_itemdata i left join gatepass_masterdata m on m.id=i.gp_master_id where m.gp_no='$gp_no' and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                                    ...
                                    group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,m.return_date,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY ORDER BY M.ID DESC");
                                    }else{
                                    $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo,returnable,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY,m.return_date
                                    ,(SELECT empname FROM vw_all_employeeinformation@crypton U WHERE U.empid = m.userid) AS USER_NAME
                                    from gatepass_itemdata i left join gatepass_masterdata m on m.id=i.gp_master_id where to_date(m.gp_date,'dd/mm/yyyy') between to_date('$date1','dd/mm/yyyy') and to_date('$date2','dd/mm/yyyy') and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                                    OR created_by IN ('$managerFeature') OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'admin' OR 
                                    (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'super admin')
                                    group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,m.return_date,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY ORDER BY M.ID DESC");
                                 }
                                    ...
                                    $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo,returnable,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY,m.return_date
                                    ,(SELECT empname FROM vw_all_employeeinformation@crypton U WHERE U.empid = m.userid) AS USER_NAME
                                    from gatepass_itemdata i left join gatepass_masterdata m on m.id=i.gp_master_id where (HEAD_OF_DEPARTMENT_STATUS = '0' or HEAD_OF_DEPARTMENT_STATUS is null) and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                                    OR created_by IN ('$managerFeature') OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'admin' OR 
                                    (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'super admin')
                                    group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,m.return_date,RECEIVED_STATUS,HEAD_OF_DEPARTMENT_STATUS,HEAD_OF_DEPARTMENT_BY ORDER BY M.ID DESC");
                                 }
                            ...
                            ?>
                                <tr class="text-center data-font">
                                    ...
                                    <?php if($auth->verifyUserPermission('gatepass', 14)){ ?>
                                    <td class="text-center data-font">
                                    <?php if ($row['HEAD_OF_DEPARTMENT_STATUS'] == 0 or $row['HEAD_OF_DEPARTMENT_STATUS']== null ){ ?>
                                        <a onclick="headOfDepartmentData(this.id)" class="button small warning fg-white rounded mif-done vw-page" id="<?php echo $row['ID'] ?>" title="Click to Receive"></a> 
                                        <?php }else{ ?>
                                        <a class="button small fg-white rounded mif-done_all" style="background-color: #004d6f;" id="<?php echo $row['ID'] ?>" title="Alrady published"></a> 
                                        <?php } ?>
                                    </td>
                                    <?php } ?>
                                    <td class="text-center data-font">
                                    ...
                                    <td class="text-center data-font"><?php echo isset($row['RETURNABLE']) ? $row['RETURN_DATE']:'Not Returnable' ?></td>
                                  
                                    <td class="text-center data-font">
                                        <a href="gatepassdetails.php?gp_master_id=<?php echo $row['ID'] ?>"  title="View Gatepass" target="_blank"><span class="mif-eye mr-2"></span></a>
                                        <?php if($row['HEAD_OF_DEPARTMENT_STATUS'] != 1){ ?>
                                        <a href="editGatepass.php?gp_master_id=<?php echo $row['ID'] ?>"  title="Edit Gatepass" target="_blank"><span class="mif-pencil mr-2"></span></a>
                                        <?php } ?>
