        ...
        <div class="navview-content h-100">
            ...
            <div class="content-inner h-100" style="overflow-y: auto">
                <div class="row border-bottom bd-lightGray pl-1 mr-1 ribbed-lightGray" style="margin-left: 0px;">
                    <div class="cell-md-4">
                        <h4 class="dashboard-section-title text-center text-left-md w-100 content-title-big" data-group="work-order" data-page="<?=$pageOpt->currentPageClass()?>" style="font-size: 1.2rem; line-height: 1; font-weight: bold; text-shadow: 1px 1px 2px #fff;"><span class="icon"><span class="mif-assignment"></span></span> Gate Pass Inventory</h4>
                    </div>
                    <div class="cell-md-8">
                    ...
                    <!-- <div class="container" id="print-js"> -->
                        ...
                        <!-- <table class="table row-border cell-border border row-hover cell-hover subcompact" data-rownum="true" data-role="table"> -->
                           ...
                           if(isset($_POST['submit'])){
                            if(!empty($gp_no)){
                                $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo
                                ...
                                where HEAD_OF_DEPARTMENT_STATUS=1 and m.gp_no='$gp_no' and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                                ...
                                group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,RECEIVED_STATUS,inventory_approved_status,head_of_department_status,approved_status");
                            }else{
                               $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo
                               ,returnable,RECEIVED_STATUS,inventory_approved_status,head_of_department_status,approved_status
                               ,(SELECT empname FROM vw_all_employeeinformation@crypton U WHERE U.empid = m.userid) AS USER_NAME
                               from gatepass_itemdata i left join gatepass_masterdata m on m.id=i.gp_master_id
                               where HEAD_OF_DEPARTMENT_STATUS=1 and to_date(m.gp_date,'dd/mm/yyyy') between to_date('$date1','dd/mm/yyyy') and to_date('$date2','dd/mm/yyyy') and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                               OR created_by IN ('$managerFeature') OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'admin' OR 
                               (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'super admin')
                               group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,RECEIVED_STATUS,inventory_approved_status,head_of_department_status,approved_status");
                           }
                                    ...
                                    $data=$accessoriesModel->getData("select m.id,to_recipient,m.gp_no,gp_date,m.userid,total_qty,address,LISTAGG(orderinformation,',') within group(order by i.id asc) as orderinfo
                                    ,returnable,RETURN_DATE,RECEIVED_STATUS,inventory_approved_status,head_of_department_status,approved_status
                                    ,(SELECT empname FROM vw_all_employeeinformation@crypton U WHERE U.empid = m.userid) AS USER_NAME
                                    from gatepass_itemdata i left join gatepass_masterdata m on m.id=i.gp_master_id
                                    where HEAD_OF_DEPARTMENT_STATUS=1 and (INVENTORY_APPROVED_STATUS='0' or INVENTORY_APPROVED_STATUS is null) and (m.DELETE_STATUS=0 or m.delete_status is null) and (created_by = '$userId'
                                    OR created_by IN ('$managerFeature') OR (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'admin' OR 
                                    (SELECT vrole FROM ACCESSORIES_USERSPERMISSION WHERE vfklid = '$userId') = 'super admin')
                                    group by m.gp_no,to_recipient,gp_date,m.userid,total_qty,address,m.id,returnable,RETURN_DATE,RECEIVED_STATUS,inventory_approved_status,head_of_department_status,approved_status order by m.gp_no desc");
                           }
                                    $i=0;
                                    foreach($data as $row){
                                        $i++;
                            ?>
                                <tr class="text-center data-font">
                                    ...
                                    <?php if ($row['INVENTORY_APPROVED_STATUS'] == 0 or $row['INVENTORY_APPROVED_STATUS']== null ){ ?>
                                        <a onclick="inventoryData(this.id)" class="button small warning fg-white inventory-page rounded mif-done" id="<?php echo $row['ID'] ?>" title="Click to approve by inventory"></a> 
                                        <?php }else{ ?>
