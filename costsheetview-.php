        ...
        <div class="navview-content h-100">
            ...
            <div class="content-inner h-100" style="overflow-y: auto">
                <div class="row border-bottom bd-lightGray pl-1 mr-1 ribbed-lightGray" style="margin-left: 0px;">
                    <div class="cell-md-4">
                        <h4 class="dashboard-section-title text-center text-left-md w-100 content-title-big" data-group="work-order" data-page="<?=$pageOpt->currentPageClass()?>" style="font-size: 1.2rem; line-height: 1; font-weight: bold; text-shadow: 1px 1px 2px #fff;"><span class="icon"><span class="mif-assignment"></span></span> Cost Sheet</h4>
                    </div>
                    <div class="cell-md-8">
                    ...
                    <!-- <div class="container" id="print-js"> -->
                        ...
                        <!-- <table class="table row-border cell-border border row-hover cell-hover subcompact" data-rownum="true" data-role="table"> -->
                            <table class="table table-border cell-border cell-hover row-hover subcompact mt-1 accessories-table-common published-workorder-table"
                                ...
                                data-table-info-title="Showing from $1 to $2 of $3 Cost Sheet List"
                                data-on-draw="preloaderClose()"
                                data-on-data-load="preloaderStart()"
                                data-on-table-create = "">
                            <thead>
                                <!-- <th>Sl</th>						 -->
                                <th class="text-center sortable-column">Publish</th>						
...
as offerprice,published_status,approval_status,revise_status,MPM_OFFER_PRICE,
                                ...
                                FROM inv.
                                    MRD_PRECOSTING_MASTER  MPM WHERE DELETE_STATUS=0 AND (MPM_EUSER = '$userId'
                            ...
                            ?>
                                <tr class="text-center">
                                    ...
                                    <?php if ($row['PUBLISHED_STATUS'] == 0 or $row['PUBLISHED_STATUS']== null ){ ?>
                                        <a onclick="publishData(this.id)" class="button small warning fg-white rounded mif-done" id="<?php echo $row['MPM_NO'] ?>" title="Click to publish"></a> 
                                        <?php }else{ ?>
                                        <a class="button small success fg-white rounded mif-done_all" id="<?php echo $row['MPM_NO'] ?>" title="Alrady published"></a> 
                                        <?php } ?>
                                    </td>
                                    <td class="text-center">
                                       <strong><?php echo $row['MPM_NO'] ?></strong> 
                               
                                    </td>
                                    <td class="text-center pos-relative ">
                                        <?php  $rev = $row['REVISE_STATUS']?$row['REVISE_STATUS']:'';
                                            if(!empty($rev)){
                                        ?>
                                        <strong class="fg-blue"><?php echo 'REV-'.$rev; ?></strong>
                                          <!-- <small class="tally info" style="left: 140px;bottom: 10px;transform: rotate(-38deg);font-size: 11px;font-weight: bold; padding: 0 4px;">REV-<?php //echo $rev; ?></small> -->
                                          <?php } ?>
                                    </td>
                                    ...
                                    <td class="text-center ">
                                        <!-- <small class="tally info" style="position: absolute;left: 140px;bottom: 10px;transform: rotate(-38deg);font-size: 11px;font-weight: bold; padding: 0 4px;">REV-1</small>
                                        <br> -->
                                        <a href="costreport.php?master_id=<?php echo $row['MPM_NO'] ?>" target="_blank"><?php echo $row['MRD_BUYER_STYLE_ID'] ?></a>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <a href="costreport.php?master_id=<?php echo $row['MPM_NO'] ?>"  title="View Cost Sheet" target="_blank"><span class="mif-eye"></span></a>
    ...
    $(document).ready(function() {
        $('.delete_btn').click(function() {
            ...
            if(result){
                ...
                var deleteId = $(this).data('id'); // Get the data-id attribute value
                $.ajax({
                    type: 'POST',
                    url: "insert.php?dailyentrydelete=ok",
